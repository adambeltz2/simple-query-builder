...
*   **Testing (dev-only exception):** `package.json`, `playwright.config.js`, and `tests/` hold a Playwright end-to-end suite that drives `index.html` in a real browser. This is a dev-time-only exception to the "no `node_modules`, no package manager" rule above — it never ships to users and `index.html` itself must remain runnable with zero install. Run tests with `npm install && npm test`; the suite serves the file over a local static server (`python3 -m http.server`, no bundler) and asserts against the rendered DOM. Add new specs under `tests/` for any user-facing behavior change to `index.html`; do not introduce a build step, transpiler, or framework dependency to make testing easier — the app being tested must stay plain HTML/CSS/JS.
